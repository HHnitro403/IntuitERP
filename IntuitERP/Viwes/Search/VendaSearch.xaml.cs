...
public partial class VendaSearch : ContentPage
{
    ...
    private async Task LoadVendasAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            await DisplayAlert("Erro", $"N�o foi poss�vel carregar a lista de vendas: {ex.Message}", "OK");
        }
    }
    ...
    private async void ExcluirVendaSelecionadaButton_Clicked(object sender, EventArgs e)
    {
        ...
        bool confirm = await DisplayAlert("Confirmar Exclus�o",
            $"ATEN��O: Esta a��o � PERMANENTE e n�o pode ser desfeita. Se a venda foi Faturada, o estoque N�O ser� revertido automaticamente.\n\nDeseja excluir a venda C�d: {_vendaSelecionada.CodVenda}?",
        ...
        if (confirm)
        {
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    await DisplayAlert("Sucesso", "Venda e seus itens foram exclu�dos permanentemente.", "OK");
                    await LoadVendasAsync(); // Refresh the list
                }
                else
                {
                    await DisplayAlert("Erro", "N�o foi poss�vel excluir o registro principal da venda (os itens podem ter sido exclu�dos).", "OK");
                }
            }
            catch (Exception ex)
            {
