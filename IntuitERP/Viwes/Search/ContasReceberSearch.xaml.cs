...
public partial class ContasReceberSearch : ContentPage, INotifyPropertyChanged
{
    ...
    public ContasReceberSearch(ContaReceberService contaService, ParcelaReceberService parcelaService)
    {
        InitializeComponent();
        BindingContext = this;
    ...
    private async void NovaContaButton_Clicked(object sender, EventArgs e)
    {
        try
        {
            // Navigate to creation page (will be implemented)
            var page = new CadastroContaReceber(_contaService, _parcelaService);
            await Navigation.PushAsync(page);
        }
        catch (Exception ex)
        {
            await DisplayAlert("Erro", $"Erro ao criar nova conta: {ex.Message}", "OK");
        }
    }
    ...
    private async void VerParcelasButton_Clicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            // Build parcelas list string
            var parcelasInfo = string.Join("\n", parcelas.Select(p =>
                $"#{p.NumeroParcela}: {p.DataVencimento:dd/MM/yyyy} - R$ {p.ValorTotal:N2} - {p.Status}"));

            await DisplayAlert($"Parcelas - Conta #{_contaSelecionada.Id}",
                $"Total: R$ {_contaSelecionada.ValorTotal:N2}\n" +
                $"Pago: R$ {_contaSelecionada.ValorPago:N2}\n" +
                $"Pendente: R$ {_contaSelecionada.ValorPendente:N2}\n\n" +
                $"Parcelas:\n{parcelasInfo}",
                "OK");
        }
        catch (Exception ex)
    ...
    private async void EditarContaButton_Clicked(object sender, EventArgs e)
    {
        ...
        try
        {
            var page = new CadastroContaReceber(_contaService, _parcelaService, _contaSelecionada);
            await Navigation.PushAsync(page);
        }
        catch (Exception ex)
        {
