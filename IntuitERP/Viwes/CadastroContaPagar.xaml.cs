...
public partial class CadastroContaPagar : ContentPage
{
    private readonly ContaPagarService _contaService;
    private readonly ParcelaPagarService _parcelaService;
    private readonly CompraService _compraService;

    private ContaPagarModel _conta;
    private CompraModel _compra;
    private ObservableCollection<ParcelaPagarModel> _parcelas;
        ...
        ContaPagarModel conta = null)
    {
        ...
        _parcelas = new ObservableCollection<ParcelaPagarModel>();
    ...
    protected override async void OnAppearing()
    {
        ...
        try
        {
            ...
            else if (_compra != null)
            {
                await LoadFromCompraAsync();
            }
            else
            {
                await DisplayAlert("Erro", "Nenhuma compra selecionada. Use esta tela através de uma compra concluída.", "OK");
    ...
    private async Task LoadFromCompraAsync()
    {
        TituloLabel.Text = "Nova Conta a Pagar";

        _conta = new ContaPagarModel
        {
            CodCompra = _compra.CodCompra,
            CodFornecedor = _compra.CodFornecedor,
            DataEmissao = _compra.data_compra ?? DateTime.Today,
            ValorTotal = _compra.valor_total,
            ValorPago = 0,
            ValorPendente = _compra.valor_total,
            NumParcelas = 1,
            Status = "Pendente"
        };

        // Load fornecedor name
        var fornecedorService = new FornecedorService(new MySqlConnectionFactory().CreateConnection());
        var fornecedor = await fornecedorService.GetByIdAsync(_compra.CodFornecedor);

        CompraLabel.Text = $"#{_compra.CodCompra} - {_compra.data_compra:dd/MM/yyyy}";
        FornecedorLabel.Text = fornecedor?.Nome ?? "Não encontrado";
        ValorTotalLabel.Text = $"R$ {_compra.valor_total:N2}";
    ...
    private async Task LoadExistingContaAsync()
    {
        ...
        // Load compra
        _compra = await _compraService.GetByIdAsync(_conta.CodCompra);

        // Load fornecedor name
        var fornecedorService = new FornecedorService(new MySqlConnectionFactory().CreateConnection());
        var fornecedor = await fornecedorService.GetByIdAsync(_conta.CodFornecedor);

        CompraLabel.Text = $"#{_conta.CodCompra} - {_conta.DataEmissao:dd/MM/yyyy}";
        FornecedorLabel.Text = fornecedor?.Nome ?? "Não encontrado";
    ...
    private async void SalvarButton_Clicked(object sender, EventArgs e)
    {
        try
        {
            ...
            // Update parcela cod_conta_pagar
            foreach (var parcela in _parcelas)
            {
                parcela.CodContaPagar = contaId;
            }

            // Insert parcelas
            await _parcelaService.InsertBatchAsync(_parcelas.ToList());

            await DisplayAlert("Sucesso", "Conta a pagar salva com sucesso!", "OK");
            await Navigation.PopAsync();
        }
        catch (Exception ex)
        {
