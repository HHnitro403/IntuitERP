...
{
    public class ParcelaPagarService
    {
        private readonly IDbConnection _connection;
        private readonly ContaPagarService _contaService;

        public ParcelaPagarService(IDbConnection connection, ContaPagarService contaService)
        {
            _connection = connection;
            _contaService = contaService;
        }

        public async Task<List<ParcelaPagarModel>> GetByContaAsync(int codContaPagar)
        {
            var sql = @"SELECT * FROM parcelas_pagar
                        WHERE cod_conta_pagar = @CodConta
                        ORDER BY numero_parcela";

            var parcelas = await _connection.QueryAsync<ParcelaPagarModel>(sql,
                new { CodConta = codContaPagar });
            return parcelas.ToList();
        }

        public async Task<ParcelaPagarModel?> GetByIdAsync(int id)
        {
            var sql = "SELECT * FROM parcelas_pagar WHERE id = @Id";
            return await _connection.QueryFirstOrDefaultAsync<ParcelaPagarModel>(sql, new { Id = id });
        }
        ...
        public async Task<int> InsertAsync(ParcelaPagarModel parcela)
        {
            ...
            var sql = @"INSERT INTO parcelas_pagar
                        (cod_conta_pagar, numero_parcela, data_vencimento, valor_parcela,
                         valor_pago, data_pagamento, forma_pagamento, status,
                         juros, multa, desconto, observacoes)
                        VALUES
                        (@CodContaPagar, @NumeroParcela, @DataVencimento, @ValorParcela,
                         @ValorPago, @DataPagamento, @FormaPagamento, @Status,
                         @Juros, @Multa, @Desconto, @Observacoes) RETURNING id;";

            return await _connection.ExecuteScalarAsync<int>(sql, parcela);
        }

        public async Task<int> UpdateAsync(ParcelaPagarModel parcela)
        {
            if (!parcela.IsValid(out string errorMessage))
            {
                throw new ArgumentException(errorMessage);
        ...
        public async Task<int> DeleteAsync(int id)
        {
            ...
            var sql = "DELETE FROM parcelas_pagar WHERE id = @Id";
            ...
            string? observacoes = null)
        {
            ...
            await _contaService.RecalcularStatusAsync(parcela.CodContaPagar);
        }
        ...
        public async Task AtualizarStatusVencimentoAsync()
        {
            ...
            var affectedCodContas = await _connection.QueryAsync<int>(@"
                SELECT DISTINCT cod_conta_pagar
                FROM parcelas_pagar
        ...
        /// </summary>
        public List<ParcelaPagarModel> GerarParcelasIguais(
            int codContaPagar,
            decimal valorTotal,
            int numParcelas,
            ...
            int intervaloDias = 30)
        {
            ...
            var parcelas = new List<ParcelaPagarModel>();
            ...
            for (int i = 1; i <= numParcelas; i++)
            {
                ...
                var parcela = new ParcelaPagarModel
                {
                    CodContaPagar = codContaPagar,
                    NumeroParcela = i,
        ...
        /// </summary>
        public async Task<int> InsertBatchAsync(List<ParcelaPagarModel> parcelas)
        {
            ...
            if (parcelas.Count > 0)
            {
                int codConta = parcelas[0].CodContaPagar;
            ...
            if (parcelas.Count > 0)
            {
                ...
                await _connection.ExecuteAsync(
                    "UPDATE contas_pagar SET num_parcelas = @NumParcelas WHERE id = @Id",
